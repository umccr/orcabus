...
class TheBusStack(Stack):
    ...
    def __init__(
        ...
        lambda_env = {"EVENT_BUS_NAME": event_bus_name}

        # Lambda to manage workflow events
        orchestrator_lambda = self.create_standard_lambda(
            scope=self,
            name="orchestrator",
        ...
        bcl_convert_lambda = self.create_standard_lambda(
            scope=self,
            name="bcl_convert",
            layers=[util_lambda_layer, schema_lambda_layer],
            env=lambda_env,
        ...
        dragen_wgs_qc_lambda = self.create_standard_lambda(
            scope=self,
            name="dragen_wgs_qc",
            layers=[util_lambda_layer, schema_lambda_layer],
            env=lambda_env,
        ...
        dragen_wgs_somatic_lambda = self.create_standard_lambda(
            scope=self,
            name="dragen_wgs_somatic",
            layers=[util_lambda_layer, schema_lambda_layer],
            env=lambda_env,
        )
        ...
        wes_launcher_lambda = self.create_standard_lambda(
            scope=self,
            name="wes_launcher",
            layers=[util_lambda_layer, schema_lambda_layer],
        )
        ...
        gds_manager_lambda = self.create_standard_lambda(
            scope=self, name="gds_manager", layers=[util_lambda_layer]
        )
        ...
        ens_manager_lambda = self.create_standard_lambda(
            scope=self,
            name="ens_event_manager",
            layers=[util_lambda_layer, schema_lambda_layer],
            env=lambda_env,
        )
        ...
        self.create_event_rule(
            ...
            event_source=EventSource.ENS_HANDLER,
        )
        ...
        self.create_event_rule(
            name="DragenWgsQc",
            bus=event_bus,
            handler=dragen_wgs_qc_lambda,
            event_type=EventType.DRAGEN_WGS_QC,
            event_source=EventSource.ORCHESTRATOR,
        )

        self.create_event_rule(
            name="DragenWgsSomatic",
            bus=event_bus,
            handler=dragen_wgs_somatic_lambda,
        ...
        self.create_event_rule(
            name="BclConvertLaunch",
            bus=event_bus,
            handler=wes_launcher_lambda,
            event_type=EventType.WES_LAUNCH,
            event_source=EventSource.BCL_CONVERT,
        )

        self.create_event_rule(
            name="DragenWgsQcLaunchRequest",
            bus=event_bus,
            handler=wes_launcher_lambda,
            event_type=EventType.WES_LAUNCH,
        ...
        self.create_event_rule(
            name="DragenWgsSomaticLaunch",
            bus=event_bus,
            handler=wes_launcher_lambda,
            event_type=EventType.WES_LAUNCH,
            event_source=EventSource.DRAGEN_WGS_SOMATIC,
        )

    # TODO: refactor to allow separate function path/name (allow better code organisation)
    def create_standard_lambda(
        self,
        scope,
        name: str,
        layers: list = [],
    ...
    def create_event_rule(
        ...
        wgs_somatic_to_wes_launcher_rule.add_target(
            target=targets.LambdaFunction(handler=handler)
        )
        wgs_somatic_to_wes_launcher_rule.add_event_pattern(
            detail_type=[event_type.value], source=[event_source.value]
        )
