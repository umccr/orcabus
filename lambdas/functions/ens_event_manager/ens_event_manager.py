...
def handle_wes_runs_event(event):
    ...
    event_action = event["messageAttributes"]["action"][
        "stringValue"
    ...
    if workflow_status == "Pending":
        ...
        raise ValueError(
            "Received non Pending WES run status! Only expect Pending at this point."
        )
...
def handle_bssh_runs_event(event):
    ...
    if srn != iri:
        raise ValueError(
            f"Sequence run name and instrumentRunId are not the same! {srn} != {iri}"
        )
