...
use crate::error::Error::{
    DbClientError, S3Error, SQSClientError, SQSDeserializeError, SQSReceiveError,
};
...
impl Receiver {
    pub fn new(sqs_client: SQSClient, sqs_url: String, s3_client: S3Client) -> Self {
        Self {
            sqs_client,
            sqs_url,
            s3_client,
    ...
    pub async fn with_default_client() -> Result<Self> {
        let config = aws_config::from_env()
            .endpoint_url(
                std::env::var("ENDPOINT_URL").map_err(|err| SQSClientError(err.to_string()))?,
            )
            .load()
            .await;
    ...
    pub async fn receive(&self) -> Result<Vec<EventMessage>> {
                ...
                .map(|message| async move {
                    ...
                    if let Some(body) = message.body() {
                        ...
                        for record in message.records.iter_mut() {
                            let head = self
                                .s3_head(&record.s3.object.key, &record.s3.bucket.name)
                                .await?;
                            record.head = head;
                        }
        ...
        .await
        .into_iter()
        .collect::<Result<Vec<EventMessage>>>()
    }
}
