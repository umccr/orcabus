...
export FILE_EVENT_SQS=file-event-sqs
export BUCKET_NAME=filemanager

echo "########### Creating upload file event SQS ###########"
aws --endpoint-url=http://localstack:4566 sqs create-queue --queue-name $FILE_EVENT_SQS

echo "########### ARN for upload file event SQS ###########"
FILE_EVENT_SQS_ARN=$(aws --endpoint-url=http://localstack:4566 sqs get-queue-attributes\
