...
export class ApiGatewayConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string) {
    ...
    this._httpApi = new HttpApi(this, 'HttpApi', {
      apiName: 'OrcaBusAPI-MetadataManager',
      corsPreflight: {
      ...
      defaultAuthorizer: this.getAuthorizer(id, 'ap-southeast-2'),
  ...
  private getAuthorizer(id: string, region: string): HttpJwtAuthorizer {
    ...
    // i.e. JWT from UI client https://status.[dev|stg|prod].umccr.org
    ...
    const SSM_STATUS_PAGE_CLIENT_ID: string = '/data_portal/status_page/cog_app_client_id_stage';
    ...
    const statusPageClientIdParam: IStringParameter =
      aws_ssm.StringParameter.fromStringParameterName(
        ...
        SSM_STATUS_PAGE_CLIENT_ID
      );

    const issuer =
      'https://cognito-idp.' + region + '.amazonaws.com/' + userPoolIdParam.stringValue;
