...
@transaction.atomic
def append_new_metadata_records(df: pd.DataFrame, df_timestamp: datetime):
    """
    Append new metadata from a pandas DataFrame into the db

    The function will check if the latest metadata record is up to the with the current df,
    ...
    for record in df.to_dict("records"):
        ...
        try:
            ...
            try:
                ...
                obj = Metadata.objects.get_single(**df_object)
                continue
            except Metadata.DoesNotExist:
                # if latest record is missing/not up to date, insert a new one
                obj = Metadata(timestamp=df_timestamp, **df_object)
                obj.save()
    ...
    return {
        "metadata_row_created_count": len(rows_created),
        "metadata_row_invalid_count": len(rows_invalid),
    }
