...
from workflow_manager.serializers import WorkflowRunCommentModelSerializer


class WorkflowRunCommentViewSet(mixins.CreateModelMixin, mixins.UpdateModelMixin, mixins.ListModelMixin,  GenericViewSet):
    queryset = WorkflowRunComment.objects.all()
    serializer_class = WorkflowRunCommentModelSerializer
    ...
    def get_queryset(self):
        return WorkflowRunComment.objects.filter(
            workflow_run=self.kwargs["workflowrun_id"],
            is_deleted=False
        ).order_by('-id')

    def create(self, request, *args, **kwargs):
        workflow_run_id = self.kwargs["workflowrun_id"]
        
        # Check if the WorkflowRun exists
        try:
            WorkflowRun.objects.get(id=workflow_run_id)
        ...
        mutable_data['workflow_run'] = workflow_run_id
