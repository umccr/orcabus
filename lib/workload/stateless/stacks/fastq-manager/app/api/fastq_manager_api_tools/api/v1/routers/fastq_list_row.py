...
from ....events.events import put_fastq_list_row_create_event, put_fastq_list_row_update_event, \
    put_fastq_list_row_delete_event
...
async def create_fastq(fastq_obj: FastqListRowCreate) -> FastqListRowResponse:
    ...
    put_fastq_list_row_create_event(fastq_dict)
...
async def update_library(fastq_id: str = Depends(sanitise_fqr_orcabus_id),
    ...
    fastq_obj_dict = fastq_obj.to_dict(include_s3_details=True)

    put_fastq_list_row_update_event(fastq_obj_dict)
...
async def add_read_count(fastq_id: str = Depends(sanitise_fqr_orcabus_id),
    ...
    fastq_dict = fastq.to_dict(include_s3_details=True)
...
async def delete_fastq(fastq_id: str) -> Dict[str, str]:
    ...
    put_fastq_list_row_delete_event(fastq_obj.id)
