...
from ....events.events import put_fastq_list_row_create_event, put_fastq_set_create_event, \
    put_fastq_list_row_update_event, put_fastq_set_update_event, put_fastq_set_delete_event, put_fastq_set_merge_event
...
async def create_fastq(fastq_set_obj_create: FastqSetCreate) -> FastqSetResponse:
    ...
    fastq_set_dict = fastq_set_data_obj.to_dict(include_s3_details=True)

    # Add in the create events
    for fastq_obj in fastq_data_objs:
        put_fastq_list_row_create_event(fastq_obj.to_dict(include_s3_details=True))

    # Add in the fastq set created event
    put_fastq_set_create_event(fastq_set_dict)
...
async def link_fastq(fastq_set_id: str = Depends(sanitise_fqs_orcabus_id), fastq_id = Depends(sanitise_fqr_orcabus_id)) -> FastqSetResponse:
    ...
    fastq_dict = fastq_obj.to_dict(include_s3_details=True)

    # Add in the updated events
    put_fastq_list_row_update_event(fastq_dict)
    put_fastq_set_update_event(fastq_set_dict)
...
async def unlink_fastq(fastq_set_id: str = Depends(sanitise_fqs_orcabus_id), fastq_id = Depends(sanitise_fqr_orcabus_id)) -> Union[FastqSetResponse, EmptyDict]:
    ...
    return fastq_set_obj.to_dict(include_s3_details=True)
...
async def merge_fastq_sets(fastq_set_ids: MergePatch = Depends()) -> FastqSetResponse:
    ...
    for fastq_set_obj in fastq_set_obj_list:
        ...
        put_fastq_set_delete_event(fastq_set_obj.id)
    ...
    for fastq_obj in fastq_obj_list:
        ...
        fastq_obj.save()
        put_fastq_list_row_update_event(fastq_obj.to_dict(include_s3_details=True))

    new_fastq_set_data_obj_dict = new_fastq_set_data_obj.to_dict(include_s3_details=True)
    put_fastq_set_merge_event({
