...
from case_manager.models import Analysis, AnalysisRun, AnalysisContext, Library
from case_manager_proc.utils import metadata_utils
import logging
...
def create_wgs_analysis(libraries: List[dict]) -> List[AnalysisRun]:
    ...
    for sbj in pairing:
        if pairing[sbj]['tumor'] and pairing[sbj]['normal']:
            ...
            analysis_run_name = f"automated__{analysis.analysis_name}__{case}__" + \
...
def create_cttso_analysis(libraries: List[dict]) -> List[AnalysisRun]:
    ...
    for lib in libraries:
        ...
        if lib['type'] == 'ctDNA' and lib['assay'] in ['ctTSO', 'ctTSOv2']:
            if lib['assay'] == 'ctTSO' and lib['case'] == 'clinical':
                analysis = v1_analysis
            else:
                analysis = v2_analysis
            if lib['case'] == 'clinical':
