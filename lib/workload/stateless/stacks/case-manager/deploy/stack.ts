...
export interface CaseManagerStackProps extends StackProps {
...
export class CaseManagerStack extends Stack {
  private props: CaseManagerStackProps;
  ...
  constructor(scope: Construct, id: string, props: CaseManagerStackProps) {
    ...
    const secretId: string = PostgresManagerStack.formatDbSecretManagerName('case_manager');
    const dbSecret = aws_secretsmanager.Secret.fromSecretNameV2(this, 'DbSecret', secretId);
    dbSecret.grantRead(this.lambdaRole);

    this.lambdaEnv = {
      DJANGO_SETTINGS_MODULE: 'case_manager.settings.aws',
  ...
  private createApiHandlerAndIntegration(props: CaseManagerStackProps) {
    ...
    const caseApi = new ApiGatewayConstruct(this, 'ApiGateway', props.apiGatewayCognitoProps);
    const httpApi = caseApi.httpApi;
  ...
  private createHandleServiceWrscEventHandler() {
    const procFn: PythonFunction = this.createPythonFunction('HandleServiceWrscEvent', {
      index: 'case_manager_proc/lambdas/handle_service_wrsc_event.py',
      handler: 'handler',
      timeout: Duration.seconds(28),
    });
    ...
    const eventRule = new Rule(this, 'EventRule', {
      description:
        'Rule to send CaseRunStateChange events to the HandleServiceWrscEvent Lambda',
