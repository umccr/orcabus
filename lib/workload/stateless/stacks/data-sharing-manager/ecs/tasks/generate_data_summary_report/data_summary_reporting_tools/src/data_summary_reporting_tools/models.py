...
class FastqModel(LibraryModel):
    ...
    isValid: Optional[bool] = pa.Field(nullable=True)
...
class WorkflowRunModel(LibraryModel):
    orcabusId: str
    currentState: StateDetail = pa.Field(coerce=True)
    workflow: Workflow = pa.Field(coerce=True)
    portalRunId: str
    executionId: Optional[str] = pa.Field(nullable=True)
    workflowRunName: str
    comment: Optional[str] = pa.Field(nullable=True)
    analysisRun: Optional[str] = pa.Field(nullable=True)
...
class SecondaryFileModel(WorkflowRunModel, FileWithRelativePathModel):
    pass
...
class FastqSummaryModel(pa.DataFrameModel):
    ...
    compression_format: str = pa.Field(alias='Compression Format')
