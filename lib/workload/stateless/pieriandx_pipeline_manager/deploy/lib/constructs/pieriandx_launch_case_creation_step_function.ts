...
interface PieriandxLaunchCaseCreationStepFunctionConstructProps {
  ...
  /* Lambdas paths */
  generate_case_lambda_path: string; // __dirname + '/../../../lambdas/upload_samplesheet_to_cache_dir'
  /* Step function templates */
  launch_pieriandx_case_creation_stepfunction_template: string; // __dirname + '/../../../step_functions_templates/launch_pieriandx_case_creation.asl.json'
  /* PierianDx env vars */
...
export class PieriandxLaunchCaseCreationStepFunctionStateMachineConstruct extends Construct {
  ...
  public readonly pieriandx_launch_case_creation_state_machine_obj: sfn.IStateMachine;
  public readonly pieriandx_launch_case_creation_state_machine_name: string;
  public readonly pieriandx_launch_case_creation_state_machine_arn: string;

  constructor(scope: Construct, id: string, props: PieriandxLaunchCaseCreationStepFunctionConstructProps) {
    super(scope, id);

    // generate_db_uuid_lambda_path lambda
    // Doesnt need any ssm parameters
    const generate_case_lambda_obj = new PythonFunction(
      this,
      'generate_case_lambda_python_function',
      {
        entry: props.generate_case_lambda_path,
    ...
    props.pieriandx_collect_access_token_lambda_obj.grantInvoke(
      // @ts-ignore
      <iam.IRole>generate_case_lambda_obj.role,
    );
    ...
    const stateMachine = new sfn.StateMachine(this, 'pieriandx_launch_step_functions_state_machine', {
      // defintiontemplate
      definitionBody: DefinitionBody.fromFile(props.launch_pieriandx_case_creation_stepfunction_template),
      // definitionSubstitutions
      definitionSubstitutions: {
        '__generate_case_lambda__': generate_case_lambda_obj.functionArn,
    ...
    // Set outputs
    this.pieriandx_launch_case_creation_state_machine_name = stateMachine.stateMachineName;
    this.pieriandx_launch_case_creation_state_machine_arn = stateMachine.stateMachineArn;
    this.pieriandx_launch_case_creation_state_machine_obj = stateMachine;
  }
