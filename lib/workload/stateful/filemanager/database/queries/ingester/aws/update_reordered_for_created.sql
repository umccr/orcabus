-- Update the matching s3_object which should be re-ordered based on the created event. Returns the
...
with current_s3_object as (
    ...
    from s3_object where
        ...
        s3_object.deleted_sequencer > $4::text and
        (
            s3_object.created_sequencer is null or
            s3_object.created_sequencer < $4::text
            )
        -- Lock this pre-emptively for the update.
        for update
), update as (
    update s3_object
        set created_sequencer = $4::text,
