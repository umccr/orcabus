...
from .globals import BSSH_ILMN_RUNS_PATH
...
def get_run_info_xml_file_id_folder_list(analysis_output: List[ProjectData]) -> str:
    return get_file_by_file_name_from_project_data_list("RunInfo.xml", analysis_output).data.id


def get_samplesheet_path_from_folder_list(analysis_output: List[ProjectData]) -> str:
    return get_file_by_file_name_from_project_data_list("SampleSheet.csv", analysis_output).data.details.path


def get_samplesheet_file_id_from_folder_list(analysis_output: List[ProjectData]) -> str:
    return get_file_by_file_name_from_project_data_list("SampleSheet.csv", analysis_output).data.id


def get_fastq_list_csv_file_id_from_folder_list(analysis_output: List[ProjectData]) -> str:
    return get_file_by_file_name_from_project_data_list("fastq_list.csv", analysis_output).data.id


def get_instrument_run_bcl_folder_id(project_id: str, v1_pre3_id: str) -> str:
    try:
        instrument_run_path_project_data_obj: ProjectData = next(filter(
            lambda project_data: project_data.data.details.name.endswith(v1_pre3_id),
            list_project_data_non_recursively(
                project_id=project_id,
                parent_folder_path=BSSH_ILMN_RUNS_PATH,
            )
        ))
        return instrument_run_path_project_data_obj.data.id
    except StopIteration:
        logger.error(f"Could not find the instrument run folder for {v1_pre3_id}")
        raise